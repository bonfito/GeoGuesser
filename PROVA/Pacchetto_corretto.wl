...
(* ============================================================== *)
(* RilevaColori                                                   *)
(* Restituisce una Association con la palette del tema corrente.  *)
(* Rileva se il notebook e' in dark o light mode confrontando     *)
(* il colore di sfondo del notebook con il nero puro.             *)
(* Viene chiamata all'avvio di ogni DynamicModule tramite         *)
(* inizializzazione nella lista delle variabili locali:           *)
(*   colori = RilevaColori[]                                      *)
(* Cosi' entrambe le interfacce (GeneraInterfaccia e             *)
(* MostraClassificaGUI) ottengono la propria palette locale       *)
(* senza variabili globali e senza Dynamic annidati.              *)
(* ============================================================== *)
RilevaColori[] := If[
  ColorDistance[CurrentValue[EvaluationNotebook[], Background], GrayLevel[0]] < 0.4,

  (* DARK MODE: testo chiaro su sfondi scuri *)
  <|
    "testoNormale"     -> GrayLevel[0.92],          (* testo generico *)
    "testoBottone"     -> White,                     (* testo su bottoni colorati *)
    "testoSuChiaro"    -> GrayLevel[0.92],           (* testo su sfondi chiari (es. bottoni bonus) *)
    "sfondoPrimario"   -> RGBColor[0.15, 0.35, 0.75], (* blu principale *)
    "sfondoBottone"    -> GrayLevel[0.30],           (* bottoni bonus / neutri *)
    "sfondoInput"      -> GrayLevel[0.20],           (* InputField *)
    "sfondoArancio"    -> RGBColor[0.8, 0.5, 0.0],  (* bottone Suggerimento *)
    "sfondoViola"      -> RGBColor[0.4, 0.1, 0.6],  (* bottone Mostra soluzione *)
    "sfondoSecondario" -> RGBColor[0.2, 0.5, 0.7],  (* bottone Pulisci Campi *)
    "sfondoConferma"   -> RGBColor[0.2, 0.6, 0.8],  (* bottone Si Salva classifica *)
    "errore"           -> RGBColor[1.0, 0.4, 0.4],  (* errori, sconfitta, No Modifica *)
    "punteggio"        -> Cyan,                      (* valore punteggio *)
    "trattino"         -> GrayLevel[0.55],           (* "_" non ancora indovinato *)
    "tastoCorretto"    -> RGBColor[0.2, 0.7, 0.3],  (* tasto tastiera: lettera corretta *)
    "tastoSbagliato"   -> RGBColor[0.75, 0.15, 0.15], (* tasto tastiera: lettera sbagliata *)
    "tastoNeutro"      -> RGBColor[0.15, 0.35, 0.75], (* tasto tastiera: non ancora usato *)
    "vittoria"         -> Lighter[Green, 0.3],       (* "Hai vinto!", punteggio salvato *)
    "bonus"            -> Lighter[Purple, 0.3],      (* titolo domanda bonus *)
    "avviso"           -> RGBColor[1.0, 0.5, 0.3],  (* avviso conferma nome *)
    "messaggio"        -> RGBColor[0.4, 0.7, 1.0]   (* feedback lettera corretta/sbagliata *)
  |>,

  (* LIGHT MODE: testo scuro su sfondi chiari *)
  <|
    "testoNormale"     -> Black,
    "testoBottone"     -> White,
    "testoSuChiaro"    -> Black,
    "sfondoPrimario"   -> RGBColor[0.1, 0.4, 0.9],
    "sfondoBottone"    -> GrayLevel[0.88],
    "sfondoInput"      -> GrayLevel[0.93],
    "sfondoArancio"    -> RGBColor[0.8, 0.5, 0.0],
    "sfondoViola"      -> RGBColor[0.4, 0.1, 0.6],
    "sfondoSecondario" -> RGBColor[0.2, 0.5, 0.7],
    "sfondoConferma"   -> RGBColor[0.3, 0.7, 0.9],
    "errore"           -> Red,
    "punteggio"        -> Blue,
    "trattino"         -> Gray,
    "tastoCorretto"    -> RGBColor[0.2, 0.7, 0.3],
    "tastoSbagliato"   -> RGBColor[0.75, 0.15, 0.15],
    "tastoNeutro"      -> RGBColor[0.15, 0.35, 0.75],
    "vittoria"         -> Darker[Green],
    "bonus"            -> Purple,
    "avviso"           -> Darker[Red],
    "messaggio"        -> RGBColor[0.1, 0.4, 0.9]
  |>
];
...
(* Un unico Dynamic con TrackedSymbols tiene traccia di tutte     *)
(* le variabili che possono cambiare \[LongDash] nessun Dynamic annidato.   *)
(*   InputField[Dynamic[nomeUtente]] usa Dynamic come binding     *)
(*   bidirezionale (lettura+scrittura): non e' annidamento,       *)
(*   e' il meccanismo obbligatorio per i campi di input.          *)
(*                                                                *)
(* Parametri:                                                     *)
...
MostraClassificaGUI[score_Integer] := DynamicModule[
{
  ...
  (* colori viene inizializzato al momento del display del DynamicModule.
     RilevaColori[] legge il tema corrente del notebook (dark/light)
     e restituisce la palette appropriata come Association. *)
  colori = RilevaColori[]
},
  ...
  (* Carica i dati esistenti dal file JSON all'apertura del dialog *)
  ...
  CreateDialog[
    Framed[
      ...
      (* Un solo Dynamic avvolge l'intera UI del dialog.
         TrackedSymbols dichiara esplicitamente le tre variabili
         che possono cambiare durante l'interazione:
           - datiClassifica : cambia dopo il salvataggio (fase 2->3)
           - faseClassifica : cambia ad ogni avanzamento di fase (1->2->3)
           - nomeUtente     : cambia ad ogni tasto premuto nell'InputField
         Ogni volta che una di queste cambia, Dynamic rivaluta
         l'intero Column \[LongDash] nessun Dynamic annidato necessario *)
      Dynamic[
        Column[{
          ...
          (* Pane crea un contenitore a dimensione fissa con scrollbar:
             {Automatic, 250} = larghezza automatica, altezza max 250px.
             Si aggiorna automaticamente quando datiClassifica cambia *)
          Pane[
            (* Grid costruisce la tabella con intestazioni e dati.
               Prepend aggiunge la riga header in cima.
               MapIndexed aggiunge il numero di riga (#2[[1]]).
               Lookup accede ai campi con valore di fallback se mancano *)
            Grid[
              Prepend[
                MapIndexed[
                  {
                    Style[#2[[1]], 16],
                    Style[Lookup[#, "nome", "N/A"], 16],
                    Style[Lookup[#, "punteggio", 0], Bold, 16]
                  } &,
                  datiClassifica
                ],
          ...
          (* ---- CONTROLLI A STATI ---- *)
          (* Switch seleziona il blocco di controlli in base alla fase.
             Rivalutato automaticamente dal Dynamic esterno quando
             faseClassifica cambia \[LongDash] nessun Dynamic interno necessario *)
          Switch[faseClassifica,
            ...
            Column[{
              ...
              (* InputField[Dynamic[nomeUtente], ...]:
                 Dynamic qui \[EGrave] un binding bidirezionale \[LongDash] non e' un
                 Dynamic annidato ma il meccanismo obbligatorio di
                 InputField per leggere e scrivere la variabile.
                 ContinuousAction->True aggiorna nomeUtente ad ogni tasto,
                 cosi' il Dynamic esterno rivaluta Enabled in tempo reale *)
              InputField[Dynamic[nomeUtente], String,
                FieldSize -> 25,
                ContinuousAction -> True
              ],
              (* Enabled usa nomeUtente direttamente \[LongDash] il Dynamic esterno
                 con TrackedSymbols:{nomeUtente} rivaluta gia' tutto
            ...
            Column[{
              (* Mostra il nome inserito per la conferma.
                 <> e' l'operatore di concatenazione stringhe *)
              Style["Sei sicuro che '" <> nomeUtente <> "' sia corretto?",
                colori["avviso"], Bold, 16],
              Spacer[10],
              Row[{
                (* Si, Salva: salva il record, ricarica la classifica
                   e avanza alla fase 3.
                   nomeUtente e' stabile \[LongDash] nessun Module necessario *)
                Button["Si, Salva",
                  SalvaRecord[nomeUtente, score];
                  datiClassifica = RecuperaClassifica[]; (* aggiorna tabella *)
                  faseClassifica = 3,                    (* avanza alla fase 3 *)
                  Background -> colori["sfondoConferma"]
                ],
                Spacer[10],
                (* No, Modifica: torna alla fase 1 senza salvare *)
                Button["No, Modifica",
                  faseClassifica = 1,
                  Background -> colori["errore"]
                ]
              }]
            }, Alignment -> Center],
            ...
            3,
            Column[{
              Style["Punteggio salvato con successo!", colori["vittoria"], Bold, 18],
              Spacer[5],
        ...
        (* TrackedSymbols elenca le variabili monitorate dal Dynamic:
           - nomeUtente     : aggiorna Enabled del bottone ad ogni tasto
           - faseClassifica : aggiorna i controlli ad ogni cambio fase
           - datiClassifica : aggiorna la tabella dopo il salvataggio *)
        TrackedSymbols :> {nomeUtente, faseClassifica, datiClassifica}
      ],

      FrameMargins -> 20,
      FrameStyle -> None,
      RoundingRadius -> 10,
      Background -> colori["sfondoPrimario"]
    ],
...
(*   colori            : Association palette dark/light mode      *)
(* ============================================================== *)
GeneraInterfaccia[] := DynamicModule[
  {
    ...
    (* colori viene inizializzato al momento del display del DynamicModule.
       RilevaColori[] legge il tema corrente del notebook (dark/light)
       e restituisce la palette appropriata come Association.
       Tutti i colori dell'interfaccia vengono letti da questa palette:
       cosi' basta cambiare RilevaColori[] per aggiornare tutto il tema. *)
    colori = RilevaColori[]
  },
 ...
 (* Dynamic avvolge Switch: rivaluta l'intera UI ogni volta
  ...
  Dynamic[
    Switch[fase, 
      ...
      "selezione", 
      Column[{
        Style["Seleziona la difficolt\[AGrave]", Bold, colori["testoNormale"], 20], 
        
        (* RadioButtonBar crea i bottoni radio collegati a gamemode.
           Dynamic[gamemode] aggiorna la variabile al click *)
        Row[{
         RadioButton[Dynamic[gamemode], 1], Style[" Facile  ", colori["testoNormale"], 16],
         RadioButton[Dynamic[gamemode], 2], Style[" Media   ", colori["testoNormale"], 16],
         RadioButton[Dynamic[gamemode], 3], Style[" Difficile", colori["testoNormale"], 16]
           }], 
        ...
        Row[{Style["Seed (opzionale): ", colori["testoNormale"]], InputField[
            ...
            FieldSize -> {15, 1}, Background -> colori["sfondoInput"], BaseStyle -> {colori["testoNormale"]}
        ]}], 
        
        (* Mostra il messaggio di errore solo se seedError non e' vuoto *)   
        If[seedError != "", Style[seedError, colori["errore"], Italic], ""],
        ...
        Button["Inizia partita", 
          ...
          BaseStyle -> {FontSize -> 16, Bold, colori["testoBottone"]}, (* Stile testo bottone *)
          Background -> colori["sfondoPrimario"]                     (* Colore sfondo bottone *)
        ]
      ...
      "gioco", 
      Column[{
        Style["Gioco dell'impiccato", Bold, 28, colori["testoNormale"]],
        
        Spacer[10],

        (* Punteggio: rivalutato dall'esterno Dynamic[Switch[...]] *)
        Row[{Style["Punteggio: ", colori["testoNormale"], 14], Style[score, colori["punteggio"], Bold, 14]}], 
        ...
        Row[Riffle[
          Which[
            ...
            # === "_", Style[" _ ", colori["trattino"], Bold, 28],  (* da indovinare *)
            True,      Style[#, Bold, 28, colori["testoNormale"]]   (* lettera indovinata *)
           ] & /@ stato,
           " "
        ]], 
        ...
        Row[{
          ...
          Button["Suggerimento", 
            ...
            BaseStyle -> {colori["testoBottone"], Bold, FontSize -> 13},
            Background -> colori["sfondoArancio"],
            ImageSize -> {140, 38}
          ],
          Spacer[20], 
        ...
        Row[{Style["Lettere sbagliate: ", colori["testoNormale"]], Style[StringJoin[Riffle[errori, ", "]], colori["errore"], Bold]}],
      
        (* Contatore errori: rivalutato dall'esterno Dynamic *)
        Row[{Style["Errori: ", colori["testoNormale"]], Style[Length[errori], colori["errore"], Bold], Style["/", colori["testoNormale"]], Style[maxErrori, colori["sfondoArancio"], Bold]}],
        
        (* Messaggio: rivalutato dall'esterno Dynamic *)
        Style[messaggio, colori["messaggio"], Bold], 

        (* Impiccato: rivalutato dall'esterno Dynamic *)
        DisegnaImpiccato[Length[errori]], 
              ...
              Row[
                Join[
                  ...
                  Table[
                    ...
                    With[{l = lettera}, 
                      Button[
                        Style[l, colori["testoBottone"], Bold, FontSize -> 16],
                        ...
                        Background -> Which[
                          MemberQ[stato, ToLowerCase[l]], colori["tastoCorretto"],  (* Verde: lettera indovinata *)
                          MemberQ[errori, ToLowerCase[l]], colori["tastoSbagliato"], (* Rosso: lettera sbagliata *)
                          True, colori["tastoNeutro"]                               (* Blu: lettera non ancora usata *)
                        ],
            ...
            Column[{
              If[stato === parola, 
                ...
                If[!faseBonusCompletata,
                  ...
                  Column[{
                    Spacer[10],
                    Style["Hai vinto!", colori["vittoria"], Bold, 18],
                    Style["Domanda Bonus (+50 punti extra!):", colori["bonus"], Bold, 18],
                    Spacer[10],
                    (* Capitalize porta la prima lettera in maiuscolo per la visualizzazione *)
                    Row[{
                      Style["Qual \[EGrave] la capitale di ", colori["testoNormale"], 16],
                      Style[Capitalize[StringJoin[parola]], colori["testoNormale"], Bold, 16],
                      Style["?", colori["testoNormale"], 16]
                    }],
                    
                    (* Generazione bottoni Bonus a scelta multipla.
                       NOTA: Background e ImageSize sono opzioni di Button (dopo la virgola
                       che chiude l'azione), non parte dell'azione stessa. *)
                    Row[Riffle[
                      Button[
                        Style[Capitalize[#], colori["testoSuChiaro"], Bold, FontSize -> 13],
                        ...
                        Background -> colori["sfondoBottone"],
                        ImageSize -> {120, 36}
                      ] & /@ opzioniBonus, 
                      Spacer[10] 
                    ]]
                  ...
                  Column[{
                    ...
                    Style[messaggioBonus, If[StringContainsQ[messaggioBonus, "punti extra"], colori["vittoria"], colori["errore"]], Bold],
                    Spacer[10],
                ...
                Column[{
                  Spacer[10],
                  Style["Hai perso! La nazione era: " <> Capitalize[StringJoin[parola]], colori["errore"], Bold],
                  Spacer[10],
                  Button["Salva e Mostra Classifica",
                    SessionSubmit[MostraClassificaGUI[score]],
                    BaseStyle -> {colori["testoBottone"], Bold, FontSize -> 13},
              ...
              Button["Pulisci Campi",
                ...
                opzioniBonus = {}; faseBonusCompletata = False; messaggioBonus = "";,
                BaseStyle -> {colori["testoBottone"], Bold, FontSize -> 13},
                Background -> colori["sfondoSecondario"],
                ImageSize -> {150, 38}
              ]
            }],
